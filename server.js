...
http.listen(4000, function () {
	...
	mongoClient.connect("mongodb://localhost:27017", function (error, client) {
		...
		app.get("/rajeshwala", function (request, result) {
			result.render("rajeshwala");
		});

		app.post("/rajeshwala", function (request, result) {
		...
		app.post("/pages", function (request, result) {
			var name = request.fields.name;
			var discription = request.fields.discription;
			var address = request.fields.address;
			var reset_token = "";

			 
						database.collection("clubdb").insertOne({
							"club_name": club_name,
							"discription": "",
							"address": "",
							...
							result.json({
								"status": "success",
								"message": "Congratulation! Your Club has been created."
							});
						});
		}	);
		...
		app.get("/club_profile", function (request, result) {
			result.render("club_profile");
		});
